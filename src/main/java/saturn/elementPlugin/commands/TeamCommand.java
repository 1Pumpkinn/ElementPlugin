 ...
 * Usage: /team <create|invite|accept|leave|kick|disband|edit|ally|list>
...
public class TeamCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleHide(Player p) {
        if (trust.isTeamHidden(p.getUniqueId())) {
            p.sendMessage(Component.text("Your team name is already hidden!", NamedTextColor.YELLOW));
            return;
        }
        trust.setTeamHidden(p.getUniqueId(), true);
        p.sendMessage(Component.text("✓ Your team name is now hidden from the tablist and displays.", NamedTextColor.GREEN));
    }
    private void handleUnhide(Player p) {
        if (!trust.isTeamHidden(p.getUniqueId())) {
            p.sendMessage(Component.text("Your team name is already visible!", NamedTextColor.YELLOW));
            return;
        }
        trust.setTeamHidden(p.getUniqueId(), false);
        p.sendMessage(Component.text("✓ Your team name is now visible everywhere.", NamedTextColor.GREEN));
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            List<String> teamCommands = Arrays.asList(
                    "list", "create", "invite", "accept", "join", "leave",
                    "kick", "disband", "edit", "ally",
                    "hide", "unhide"
            );
